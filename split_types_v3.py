#!/usr/bin/env python3
"""
Split google/genai/types.py into multiple organized files (v3 - split base.py).
"""
...
for class_name, start, end in class_ranges:
    ...
    # Content and messages
        ...
        filename = 'content.py'
    # Generation configs (but not routing or thinking which are more specific)
    ...
    # Configs (catch remaining *Config classes)
...
for filename in sorted(file_assignments.keys(), key=lambda x: -len(file_assignments[x])):
    count = len(file_assignments[filename])
